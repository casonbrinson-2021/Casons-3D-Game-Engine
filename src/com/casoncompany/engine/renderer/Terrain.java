...
public class Terrain {
	...
	public Terrain(Vector3f position, ObjectLoader objectLoader, Material material, BlendMapTerrain blendMapTerrain, TerrainTexture blendMap) {
	...
	private Model generateTerrain(ObjectLoader objectLoader) {
		...
		for(int i = 0; i < VERTEX_COUNT; i++) {
			for(int j = 0; j < VERTEX_COUNT; j++) {
				...
				textureCoords[vertexPointer * 2] = j / (VERTEX_COUNT - 10.f);
				textureCoords[vertexPointer * 2 + 1] = i / (VERTEX_COUNT - 10.f);
				vertexPointer++;
			}
		}
